...
describe('classify-invoices', () => {
	it('classifies outstanding as due today or earlier only', () => {
		const invoices = [
			invoice({ invoiceId: '1', status: 'unpaid', balance: 500, dueDate: '2026-07-10' }),
			invoice({ invoiceId: '2', status: 'unpaid', balance: 700, dueDate: '2026-07-31' }),
			invoice({ invoiceId: '3', status: 'paid', balance: 0, total: 500, dueDate: '2026-07-01' }),
			invoice({ invoiceId: '4', status: 'draft', balance: 200, dueDate: '2026-07-01' })
		];
	...
	it('classifies draft dated first of next month for earned pipeline', () => {
		const invoices = [
			invoice({ invoiceId: '1', status: 'draft', date: '2026-08-01', total: 2500 }),
			invoice({ invoiceId: '2', status: 'draft', date: '2026-07-10', total: 100 }),
			invoice({ invoiceId: '3', status: 'sent', date: '2026-08-01', total: 900 })
		];
	...
	it('converts mixed-currency outstanding balances into base', () => {
		const mixedFx: FxContext = { baseCurrencyCode: 'GBP', rates: { GBP: 1, EUR: 0.85 } };
		const invoices = [
			invoice({
				invoiceId: '1',
				status: 'unpaid',
