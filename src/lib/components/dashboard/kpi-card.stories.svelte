...
<Story
  ...
  play={async ({ canvasElement }) => {
    ...
    await expect(canvas.getByTestId('source-badge')).toHaveTextContent('Cash collected');
    await expect(canvas.getByTestId('kpi-card')).toHaveTextContent(/£3,100/);
    const infoHint = canvas.getByTestId('info-hint');
    await expect(infoHint).toHaveAccessibleName('How Paid this month is calculated');
...
<Story
  ...
  play={async ({ canvasElement }) => {
    ...
    const touchTooltip = await within(document.body).findByTestId('currency-amount-tooltip');
