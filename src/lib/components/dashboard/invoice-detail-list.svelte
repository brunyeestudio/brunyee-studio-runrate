...
{#if invoices.length === 0}
	<p class="text-muted-foreground text-xs" data-testid="invoice-detail-empty">{emptyMessage}</p>
{:else}
			...
			<li class="flex items-start justify-between gap-3 py-2 first:pt-0 last:pb-0">
				<div class="min-w-0 space-y-0.5">
					<p class="truncate text-xs font-medium">
						...
						<span class="text-muted-foreground font-normal">· {invoice.customerName}</span>
					</p>
