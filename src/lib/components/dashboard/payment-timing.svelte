    ...
    <Card.Header class="gap-2">
      <div class="flex items-center justify-between gap-2">
        <div class="flex items-center gap-1.5">
          ...
          <InfoHint
            label="How due end of this month is calculated"
            text={kpiInfo.dueThisMonth}
          />
        </div>
    ...
    <Card.Content class="text-muted-foreground text-xs">
      <Accordion.Root type="single" class="w-full">
        <Accordion.Item value="details" class="border-0">
          ...
          >
            {dueThisMonth.count} outstanding invoice{dueThisMonth.count === 1
              ? ''
              : 's'} expected to pay
          </Accordion.Trigger>
    ...
    <Card.Header class="gap-2">
      <div class="flex items-center justify-between gap-2">
        <div class="flex items-center gap-1.5">
          ...
          <InfoHint
            label="How due next month is calculated"
            text={kpiInfo.dueNextMonth}
          />
        </div>
      ...
      <Accordion.Root type="single" class="w-full">
        <Accordion.Item value="details" class="border-0">
          <Accordion.Trigger
            ...
            data-testid="payment-timing-details-trigger"
          >
            {dueNextMonth.count} outstanding invoice{dueNextMonth.count === 1
              ? ''
              : 's'} expected to pay
          </Accordion.Trigger>
          <Accordion.Content class="pt-2 pb-0 text-xs" forceMount={false}>
